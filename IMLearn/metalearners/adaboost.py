...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for t in range(self.iterations_):
            y_hat = self.models_[t].fit_predict(X, y * self.D_)
            epsilon = np.sum(self.D_[y_hat != y])
            if epsilon == 0:
                self.weights_[t] = 1 - np.sum(self.weights_)
                self.D_ = self.D_ * np.exp(-y * self.weights_[t] * y_hat)
                self.D_ /= np.sum(self.D_)
                break
            self.weights_[t] = .5 * np.log(1 / epsilon - 1)
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        responses : ndarray of shape (n_samples, )
            Predicted responses of given samples
        """
        prediction = [self.weights_[i] * m.predict(X) for i, m in enumerate(self.models_[:min(T, self.iterations_)])]
        return np.sign(np.sum(prediction, axis=0))
