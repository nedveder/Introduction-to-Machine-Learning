...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.penalty_ != "none":
            model = RegularizedModule(model, L1() if self.penalty_ == "l1" else L2(), lam=self.lam_,
                                      include_intercept=self.include_intercept_)
        model.weights = np.random.normal(0, 1, size=X.shape[1]) / np.sqrt(X.shape[1])
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.include_intercept_:
            X = np.insert(X, 0, 1, axis=1)
        return np.where(expit(X @ self.coefs_) >= self.alpha_, 1, 0)
